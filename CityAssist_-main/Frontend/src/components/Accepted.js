...
const Accepted = () => {
  ...
  useEffect(() => {
    const fetchAcceptedIssues = async () => {
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_URL}/admin/getAcceptedIssues`, {
          method: "GET",
          headers: {
  ...
  const handleStatusChange = async (issueId, newStatus) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/admin/updateIssueStatus/${issueId}`, {
        method: "PATCH",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
